module EmailSpec
  module Matchers
    ...
    class DeliverTo
      def initialize(recipients)
        emails = recipients.map do |recipient|
          recipient.kind_of?(String) ? recipient : recipient.email
        end
      ...
      def failure_message
        "expected #{@email.inspect} to deliver to #{@expected_email_addresses.inspect}, " + 
          "but it delivered to #{@actual_recipients.inspect}"
      end
    ...
    def deliver_to(*recipients)
      DeliverTo.new(recipients.flatten)
    end
    ...
    class DeliverFrom
      ...
      def failure_message
        "expected #{@email.inspect} to deliver from #{@expected_email_addresses.inspect}, " + 
          "but it delivered from #{@actual_sender.inspect}"
      end
    ...
    class BccTo
      ...
      def failure_message
        "expected #{@email.inspect} to bcc to #{@expected_email_addresses.inspect}, " + 
          "but it was bcc'd to #{@actual_recipients.inspect}"
      end
    ...
    def bcc_to(*recipient)
      BccTo.new(recipient.flatten)
    end
    ...
    def have_subject(expected)
      simple_matcher do |given, matcher|
        ...
        if expected.is_a?(String)
          ...
          matcher.failure_message = "expected the subject to match #{expected.inspect}, " + 
            "but did not.  Actual subject was: #{given_subject.inspect}"
          matcher.negative_failure_message = "expected the subject not to match #{expected.inspect} " + 
            "but #{given_subject.inspect} does match it."
     ...
     def include_email_with_subject(expected)
       simple_matcher do |given_emails, matcher|
         ...
         if expected.is_a?(String)
           ...
           matcher.failure_message = "expected at least one email to have the subject #{expected.inspect} " + 
             "but none did. Subjects were #{given_emails.map(&:subject).inspect}"
           matcher.negative_failure_message = "expected no email with the subject #{expected.inspect} " + 
             "but found at least one. Subjects were #{given_emails.map(&:subject).inspect}"

           given_emails.map(&:subject).include?(expected)
         else
           matcher.description = "include email with subject matching #{expected.inspect}"
           matcher.failure_message = "expected at least one email to have a subject matching #{expected.inspect}, " + 
             "but none did. Subjects were #{given_emails.map(&:subject).inspect}"
           matcher.negative_failure_message = "expected no email to have a subject matching #{expected.inspect} " + 
             "but found at least one. Subjects were #{given_emails.map(&:subject).inspect}"
     ...
     def have_body_text(expected)
       simple_matcher do |given, matcher|
         ...
         if expected.is_a?(String)
           ...
           matcher.failure_message = "expected the body to contain #{normalized_expected.inspect} " +
             "but was #{normalized_body.inspect}"
           matcher.negative_failure_message = "expected the body not to contain #{normalized_expected.inspect} " + 
             "but was #{normalized_body.inspect}"
           ...
           matcher.failure_message = "expected the body to match #{expected.inspect}, " + 
             "but did not.  Actual body was: #{given_body.inspect}"
           matcher.negative_failure_message = "expected the body not to match #{expected.inspect} " + 
             "but #{given_body.inspect} does match it."
      ...
      def have_header(expected_name, expected_value)
        simple_matcher do |given, matcher|
          ...
          if expected_value.is_a?(String)
            ...
            "but they were #{given_header.inspect}"
            matcher.negative_failure_message = "expected the headers not to include '#{expected_name}: #{expected_value}' " +
            "but they were #{given_header.inspect}"
            ...
            matcher.failure_message = "expected the headers to include '#{expected_name}' " + 
              "with a value matching #{expected_value.inspect} but they were #{given_header.inspect}"
            matcher.negative_failure_message = "expected the headers not to include '#{expected_name}' " + 
              "with a value matching #{expected_value.inspect} but they were #{given_header.inspect}"
